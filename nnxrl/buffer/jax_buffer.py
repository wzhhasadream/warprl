...
def _reshape_obs(x: Any, n_envs: int, obs_shape: tuple[int, ...], dtype: jnp.dtype) -> jax.Array:
    return jnp.asarray(x, dtype=dtype).reshape((n_envs, *obs_shape))


def _reshape_action(x: Any, n_envs: int, action_shape: tuple[int, ...], dtype: jnp.dtype) -> jax.Array:
    return jnp.asarray(x, dtype=dtype).reshape((n_envs, *action_shape))


def _reshape_scalar(x: Any, n_envs: int) -> jax.Array:
    return jnp.asarray(x, dtype=jnp.float32).reshape((n_envs,))
...
class JaxBuffer:
    ...
    def create(
        ...
        rewards = jnp.empty((max_size,), dtype=jnp.float32)
        terminations = jnp.empty((max_size,), dtype=jnp.float32)
        truncations = jnp.empty((max_size,), dtype=jnp.float32)
        next_observations = jnp.empty((max_size, *obs_shape), dtype=obs_dtype)
        discounts = jnp.empty((max_size,), dtype=jnp.float32)
        timestamps = jnp.zeros((max_size,), dtype=jnp.int32)

        window_observations = jnp.empty((n_step, n_envs, *obs_shape), dtype=obs_dtype)
        window_actions = jnp.empty((n_step, n_envs, *action_shape), dtype=action_dtype)
        window_rewards = jnp.empty((n_step, n_envs), dtype=jnp.float32)
        window_terminations = jnp.empty((n_step, n_envs), dtype=jnp.float32)
        window_truncations = jnp.empty((n_step, n_envs), dtype=jnp.float32)
        window_next_observations = jnp.empty((n_step, n_envs, *obs_shape), dtype=obs_dtype)
        ...
        return cls(
            ...
            window_ptr=jnp.array(0, dtype=jnp.int32),
            window_size=jnp.array(0, dtype=jnp.int32),
    ...
    def add(self, transition: Transition) -> "JaxBuffer":
        ...
        rewards = _reshape_scalar(transition.rewards, self.n_envs)
        terminations = _reshape_scalar(transition.terminations, self.n_envs)
        truncations = _reshape_scalar(transition.truncations, self.n_envs)
        next_observations = _reshape_obs(transition.next_observations, self.n_envs, self.obs_shape, self.obs_dtype)
    ...
    def _get_n_step_transition(self) -> tuple[Transition, jax.Array]:
        ...
        effective_n_steps = jnp.ones((self.n_envs,), dtype=jnp.float32)
        ...
        discounts = jnp.power(jnp.asarray(self.gamma, dtype=jnp.float32), effective_n_steps)
        return transition, discounts

    def _store_from_window(self) -> "JaxBuffer":
        transition, discounts = self._get_n_step_transition()
        add_indices = (self.ptr + jnp.arange(self.n_envs, dtype=self.ptr.dtype)) % self.max_size
    ...
    def reset(self) -> "JaxBuffer":
        return self.replace(
            ptr=jnp.array(0, dtype=self.ptr.dtype),
            time_size=jnp.array(0, dtype=self.time_size.dtype),
            size=jnp.array(0, dtype=self.size.dtype),
            current_time=jnp.array(0, dtype=self.current_time.dtype),
            window_ptr=jnp.array(0, dtype=self.window_ptr.dtype),
            window_size=jnp.array(0, dtype=self.window_size.dtype),
        )

    def save(self, path: str) -> None:
        dataset = {
            "observations": np.asarray(self.observations[:self.size]),
            "actions": np.asarray(self.actions[:self.size]),
            "rewards": np.asarray(self.rewards[:self.size]),
            "terminations": np.asarray(self.terminations[:self.size]),
            "truncations": np.asarray(self.truncations[:self.size]),
            "next_observations": np.asarray(self.next_observations[:self.size]),
            "discounts": np.asarray(self.discounts[:self.size]),
            "timestamps": np.asarray(self.timestamps[:self.size]),
